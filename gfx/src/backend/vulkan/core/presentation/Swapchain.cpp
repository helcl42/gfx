...
namespace {
    VkSurfaceFormatKHR chooseSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats, VkFormat requested)
    {
        for (const auto& availableFormat : formats) {
            if (availableFormat.format == requested) {
    ...
    VkPresentModeKHR choosePresentMode(const std::vector<VkPresentModeKHR>& presentModes, VkPresentModeKHR requested)
    {
        for (const auto& availableMode : presentModes) {
            if (availableMode == requested) {
    ...
    VkExtent2D chooseExtent(const VkSurfaceCapabilitiesKHR& capabilities, uint32_t requestedWidth, uint32_t requestedHeight)
    {
        if (capabilities.currentExtent.width != UINT32_MAX) {
            return capabilities.currentExtent;
        }
        return {
            std::max(capabilities.minImageExtent.width, std::min(requestedWidth, capabilities.maxImageExtent.width)),
            std::max(capabilities.minImageExtent.height, std::min(requestedHeight, capabilities.maxImageExtent.height))
        };
    }

    uint32_t chooseImageCount(const VkSurfaceCapabilitiesKHR& capabilities, uint32_t requested)
    {
        uint32_t imageCount = std::min(requested, capabilities.minImageCount + 1);
    ...
    std::vector<VkImage> fetchSwapchainImages(VkDevice device, VkSwapchainKHR swapchain)
    {
    ...
    VkCompositeAlphaFlagBitsKHR chooseCompositeAlpha(const VkSurfaceCapabilitiesKHR& capabilities)
    {
        ...
        return preferredAlphaModes[0]; // Should never happen, but return opaque as a fallback
    }

    SwapchainInfo makeSwapchainInfo(const VkExtent2D& extent, const VkSurfaceFormatKHR& format, const VkPresentModeKHR presentMode, const uint32_t imageCount)
    {
        SwapchainInfo info{};
    ...
    , m_surface(surface)
{
    ...
    VkSurfaceFormatKHR selectedFormat = chooseSurfaceFormat(formats, createInfo.format);
    VkExtent2D extent = chooseExtent(capabilities, createInfo.width, createInfo.height);
    VkPresentModeKHR presentMode = choosePresentMode(presentModes, createInfo.presentMode);
    ...
    vkCreateInfo.minImageCount = chooseImageCount(capabilities, createInfo.imageCount);
    vkCreateInfo.imageFormat = selectedFormat.format;
    ...
    vkCreateInfo.compositeAlpha = chooseCompositeAlpha(capabilities);
    vkCreateInfo.presentMode = presentMode;
    vkCreateInfo.clipped = VK_TRUE;

    // Resolve the present queue before creating the swapchain so a failure here
    // cannot leak the swapchain handle (a throwing constructor skips the destructor)
    ...
    m_images = fetchSwapchainImages(m_device->handle(), m_swapchain);
    m_info = makeSwapchainInfo(extent, selectedFormat, presentMode, static_cast<uint32_t>(m_images.size()));
...
// Wraps each fetched swapchain image in a non-owning Texture + TextureView
void Swapchain::createImageWrappers()
{
    m_textures.reserve(m_images.size());
    m_textureViews.reserve(m_images.size());
...
void Swapchain::destroyImageWrappers()
{
