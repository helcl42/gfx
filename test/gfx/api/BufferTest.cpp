...
TEST_P(GfxBufferTest, MapAsyncPollsUntilReady)
{
    GfxBufferDescriptor desc = {};
    desc.label = "Polled Map Buffer";
    desc.size = 1024;
    desc.usage = GFX_FLAGS(GFX_BUFFER_USAGE_MAP_WRITE | GFX_BUFFER_USAGE_COPY_SRC);
    ...
    EXPECT_EQ(gfxBufferMapAsync(buffer, 0, desc.size, &samePtr), GFX_RESULT_SUCCESS);
    EXPECT_EQ(samePtr, ptr);
